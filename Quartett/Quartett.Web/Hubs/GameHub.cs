...
namespace Quartett.Web.Hubs
{
    public sealed class GameHub : Hub<IGameClient>, IGameServer
    {
        ...
        public async Task ApplyChoice(string characteristicName)
        {
            ...
            Clients.Client(game.Player1.PlayerId)
                .Reveal(winnerOfRound == game.Player1.PlayerId, game.Player2.NextCard, characteristicName);
            Clients.Client(game.Player2.PlayerId)
                .Reveal(winnerOfRound == game.Player2.PlayerId, game.Player1.NextCard, characteristicName);
